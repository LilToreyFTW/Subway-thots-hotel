...
The portable deployment bundle is in `vps_connection/`. Copy that folder to `C:\Users\Administrator\Desktop\vps_connection` on the Windows VPS, then run `Install-Production-Host.ps1` as Administrator. The installer keeps port 7076 private, opens HTTPS port 443, and installs automatic startup tasks for the world server and Caddy TLS proxy. The client connects to `wss://cyan-squirrel-97200.zap.cloud`.
