...
class UpdateService {
  ...
  configure() {
    ...
    autoUpdater.on('error', (error) => this.set(makeState('ERROR', { errorMessage: error.message })));
  }
  set(state) { this.state = state; this.log?.(`[update] ${state.state} ${state.availableVersion || ''}`); this.send(this.state); }
  async check() { if (!this.production) return this.set(makeState('OFFLINE', { errorMessage: 'Development mode has no release updater.' })); this.set(makeState('CHECKING')); try { await autoUpdater.checkForUpdates(); } catch (e) { this.set(makeState('ERROR', { errorMessage: e.message })); } return this.state; }
  async download() { if (!this.production) return this.state; try { await autoUpdater.downloadUpdate(); } catch (e) { this.set(makeState('ERROR', { errorMessage: e.message })); } return this.state; }
