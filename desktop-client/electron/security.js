...
function installSecurity(gameScheme, log) {
  ...
  session.defaultSession.webRequest.onBeforeRequest({ urls: ['*://*/*'] }, (details, callback) => { if (details.url.startsWith(`${gameScheme}://`) || details.url.startsWith('https://') || details.url.startsWith('http://127.0.0.1:')) return callback({}); log?.(`[security] blocked ${details.url}`); callback({ cancel: true }); });
}
